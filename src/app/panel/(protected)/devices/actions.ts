...
const schema = z.object({
  ...
  direccion_recogida: z
    .string()
    .max(280, "La dirección es demasiado larga")
    .optional()
    .or(z.literal("")),
});
...
export async function guardarKiosko(
  ...
  const parsed = schema.safeParse({
    ...
    direccion_recogida: formData.get("direccion_recogida") ?? "",
  });
  if (!parsed.success) return { error: parsed.error.issues[0]?.message };
  const d = parsed.data;
  const direccionRecogida = d.direccion_recogida?.trim() || null;
